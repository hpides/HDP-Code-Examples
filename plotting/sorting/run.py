        ...
        for stop_run in range(5):
            ...
            for line in result.stderr.splitlines():
                if "Joules" in line:
                    ...
                    energy_results.append({"MEASUREMENT": "Data Generation", "DATASET_SIZE": item_count, "PERF_METRIC": line_split[1], "JOULES": float(line_split[0]), "RUNTIME_S": end-start})
            time.sleep(5)
            ...
            for line in result.stderr.splitlines():
                if "Joules" in line:
                    ...
                    energy_results.append({"MEASUREMENT": "Idling", "PERF_METRIC": line_split[1], "JOULES": float(line_split[0]), "RUNTIME_S": end2-start2})
            ...
            for run in range(5):
                ...
                for line in result.stderr.splitlines():
                    if "Joules" in line:
                        ...
                        energy_results.append({"MEASUREMENT": "Sorting", "CORE_COUNT": core_count, "SORT_VARIANT": sort_name, "DATASET_SIZE": item_count, "PERF_METRIC": line_split[1], "JOULES": float(line_split[0]), "RUNTIME_S": end-start})
