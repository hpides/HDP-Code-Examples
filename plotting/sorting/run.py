...
results = []
energy_results = []
for sort_name, sort_mode in [("Parallel std::sort", "-DPARALLEL_STD_SORT")]:
#for sort_name, sort_mode in [("Sequential std::sort", ""), ("Parallel std::sort", "-DPARALLEL_STD_SORT")]:
    for item_count, size_mode in [(4_000_000, "-DLARGE_DATASET")]:
    ...
    #for item_count, size_mode in [(250_000, "")]:
        ...
        for stop_run in range(5):
            time.sleep(5)
            stop_command = f"perf stat {perf_metrics} numactl -N 0 -m 0 ./sort__{hostname} STOP"
            start = time.time()
            ...
            time.sleep(5)
            idle_command = f"perf stat {perf_metrics} numactl -N 0 -m 0 sleep 10"
            start2 = time.time()
        ...
        cumu_runtime = 0.0
        for run in range(5):
            command = f"perf stat {perf_metrics} numactl -m 0 -N 0 ./sort__{hostname}"
            start = time.time()
